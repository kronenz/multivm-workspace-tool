...
├── AGENTS.md                    # AI Agent 조직 차터 (5계층 인덱스, 협업 프로토콜)
...
│  ┌─── AI Agent 조직 인프라 ─────────────────────────────────────┐
│  │                                                            │
├── .agents/                     # AI Agent 조직 운영 폴더
│   ├── README.md                # 조직 가이드 + 룩업 방법
│   ├── registry.json            # 기계 판독 인덱스 (역할, 파일 소유권, 프로토콜)
│   ├── executive/               # 1.x 전략 계층 (Product/Technical Director)
│   ├── divisions/               # 2.x 부문 계층 (Backend/Frontend/QA/Ops)
│   ├── teams/                   # 3.x 팀 계층 (SSH, Workset, Terminal 등 6팀)
│   ├── guidelines/              # 4.x 실무 가이드라인 (코드, 네이밍, Git, 체크리스트)
│   ├── support/                 # 5.x 지원 계층 (문서, 보안)
│   └── protocols/               # 9.x 협업 프로토콜 (Quick Decision, Cross-Team 등)
│  │                                                            │
│  └────────────────────────────────────────────────────────────┘
│
│  ┌─── 빌드 설정 ──────────────────────────────────────────────┐
...
│   ├── terminal.ts              # Terminal Emulator (118줄) — xterm.js v6 WebGL/DOM, FitAddon
...
2. **`AGENTS.md`** — AI Agent 조직 인덱스 (`.agents/` 폴더의 26개 파일로 연결)
...
|------|---------|---------|
| 2026-02-08 | 2.2 | .agents/ 폴더 구조 추가, 의존성 업데이트 반영 (xterm v6, vite 7, tauri 2.10), AGENTS.md v4.0 인덱스 전환 |
| 2026-02-08 | 2.1 | AGENTS.md v3.0 조직 차터 반영: 문서 설명 갱신 (프로젝트 지식 베이스 → AI Agent 조직 차터) |
